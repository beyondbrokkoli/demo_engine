...
local vk_pipeline, vk_dynamic = reg.vk_pipeline, reg.vk_dynamic
local vk_format, vk_image = reg.vk_format, reg.vk_image

local Utils = {}

function Utils.ReadShaderFile(filename)
...
function Utils.createShaderModule(vk, device, filename)
    local code = Utils.ReadShaderFile(filename)
...
function Utils.BuildSinglePipeline(vk, device, layout, colorFormat, vertModule, fragModule, config)
